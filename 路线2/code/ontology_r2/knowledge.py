...
class RetrievalModel(ChatModelBase):
    ...
    async def count_tokens(self, messages=None, tools=None, **kwargs):
        size = len(json.dumps(visible({"messages": messages, "tools": tools, **kwargs}), ensure_ascii=False, default=str).encode())
    ...
    async def _call_api(self, model_name, messages, tools=None, tool_choice=None, **kwargs):
        request = visible({"messages": messages, "tools": tools, "tool_choice": str(tool_choice)})
        if self.calls >= self.maximum:
            self.state["failure"] = "budget_exhausted"
            raise BudgetExceeded("ReAct model-call limit reached")
        try:
            self.llm.admit(request)
        ...
        if self.llm.mode == "mock":
            ...
            response = await asyncio.wait_for(self.llm.get_model()(messages, tools=tools, tool_choice=tool_choice, **kwargs), self.llm.config.get("timeout_seconds", 60))
            if response.usage:
                self.llm.actual_tokens += response.usage.input_tokens + response.usage.output_tokens
                self.llm.trace({"stage": "llm_usage", "task": "react", "usage": asdict(response.usage)})
