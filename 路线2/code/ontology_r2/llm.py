...
class StructuredLLM:
    ...
    def get_model(self):
        if self.model is None:
            ...
            self.model = OpenAIChatModel(
                ...
                model=os.environ["ONTOLOGY_LLM_MODEL"], stream=False, max_retries=0,
                parameters=OpenAIChatModel.Parameters(temperature=0, max_tokens=self.config.get("max_output_tokens", 4096)),
    ...
    async def ask(self, task, payload, schema):
        ...
        key = digest([request, model_id, self.mode, self.config])
        ...
        if file.exists():
            ...
            return result
        self.admit(request)
        self.trace({"stage": "llm_request", "mode": self.mode, "task": task, "cache_key": key, "input": payload, "schema": schema_dict})
        if self.mode == "mock":
            ...
            response = await asyncio.wait_for(self.get_model()(messages, tools=[tool], tool_choice=ToolChoice(mode="submit_result")), timeout=self.config.get("timeout_seconds", 60))
    ...
    def metrics(self):
        return {"mode": self.mode, "model": os.getenv("ONTOLOGY_LLM_MODEL") if self.mode != "mock" else "recorded_fixture", "framework": "agentscope-2.0.8", "prompt_hash": digest([SYSTEM, TASK_PROMPTS]), "calls": self.calls, "reserved_token_upper_bound": self.reserved_tokens, "provider_reported_tokens": self.actual_tokens if self.mode != "mock" else None, "cache_hits": self.cached}
