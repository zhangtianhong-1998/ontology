...
代码位于 [code/ontology_r2](code/ontology_r2)。本轮按上游 RIGOR 的直接映射、逐表增量、当前 core 上下文、Judge 修正及合并校验逐项改造，详细差异见 [RIGOR 对照](RIGOR对照与本轮改造.md)。上游源码保留原样，内部仍采用用户定义的 YAML 模型。
...
- 三目录 YAML 和 CSV 导入；原值保留为字符串，保存文件哈希、原始约束、字段说明和快照记录位置。字段统计包含非空数、近似不同值数和有范围标记的枚举样本。
...
本代码包不包含数据文件；上面的 `make-demo` 命令在本地生成 `fixtures/linked`。已存在的结果目录不会覆盖，重跑时换一个输出名称。模拟数据和手写模拟模型响应均明确标记，不代表真实业务中存在这些字段或关系。
...
先执行 `cp -n .env.example .env`，再填写 `ONTOLOGY_LLM_MODEL`、`ONTOLOGY_LLM_BASE_URL`、`ONTOLOGY_LLM_API_KEY`，变量格式见 `.env.example`。模型服务需兼容 Chat Completions 的结构化工具输出。
...
`--profile E2L/E2O/E2K/E2F` 分别选择本地、加外部本体、加 MCP、两者都加。E2O / E2F 需要先取得对应的外部本体文件，下载来源见 [Git 管理](../Git管理.md)。默认模拟流程关闭外部本体，保留模拟 MCP。运行 manifest 保存实际开关；`external.sources` 选择参与实验的模型，约定路径见 `ontologies/sources.yaml`。
...
- 字段统计仍是基础计数、近似基数和排序取值样本；尚无自动跨字段包含率/候选发现模块。下一步设计见[字段统计与关联发现设计](字段统计与关联发现设计.md)，本次文档更新没有改变这些运行行为。
...
- 本代码包不携带历史运行结果；百万条完整端到端性能不能由小样本测试代替。
...
需求与代码对应关系见 [implementation.md](implementation.md)，代码包验证方法见 [测试说明](测试说明.md)。
