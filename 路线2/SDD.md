...
本轮已按 [RIGOR 对照与本轮改造](RIGOR对照与本轮改造.md)实现确定性直接映射、逐表增量与回滚、AgentScope 原生 ReAct 检索及本地结果浏览。运行代码位于 `code/ontology_r2`，配置入口为 `config/runtime.mock.yaml`。
...
2026-09-23 新增[字段统计与关联发现详细设计](字段统计与关联发现设计.md)及[字段关联方案评估](字段关联方案评估.md)。字段统计、采样、候选生成、精确验证和预算以该详细设计为准；其中的新模块尚未实现。原型现状仍是基础统计和前 3 行样本，已有的 27 项测试不能用于证明这次新增设计已经完成。
...
计数范围区分“原库导出是否全量”和“本次是否扫描完输入”；所有近似值、采样值和未完成统计明确标记。空值策略、规范化碰撞及逐项 SQL/样本口径见[详细设计第3节](字段统计与关联发现设计.md)。
...
上述比率还需拆成“命中任意目标”和“唯一定位目标”，并与不同键包含率分别报告；列表/公式的引用数与源记录数分开。NULL、空引用、缺作用域、解析失败和预算未处理不混入同一分母。精确口径以[详细设计第4节](字段统计与关联发现设计.md)为准。
...
当前单次构建内的 core 累积、事务式接受/回滚、Judge 修正复验、直接映射覆盖和 UI 规格见 [本轮详细设计](RIGOR对照与本轮改造.md)。所有调用有可见输入、结构化输出和工具轨迹，不记录模型隐藏思维。
