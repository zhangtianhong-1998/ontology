...
实际流程：导入与分组字段统计 → 有界字段候选及精确核验、可选定义记录和别名候选召回 → 全表全字段直接映射 → 按声明依赖遍历工作单元 → 按列角色压缩语义提示词、选取少量已核验候选的联合行例与定义记录候选 → 可选企业检索及外部本体对齐 → 生成 delta → 程序校验与 Judge 复核 → 接受增量或保留旧 core → 按计划抽取记录 → 本地结果页面。候选和联合行例都不是已接受的业务关系。

增量构建只遍历一次表目录，每张表对应一个工作单元；23 张表就是 23 个单元。每表最多尝试 `llm.max_repairs + 1` 次；两个真实模型配置 `runtime.real.example.yaml`、`runtime.no-thinking.yaml` 都设为 `max_repairs: 4`，即每表最多 5 次。每次先生成并校验增量，校验通过后交给 Judge；成功后立即进入下一表，失败则保留原 core，不会自动回访。若调用超时，同一提示词不会再作为语义修复重复发送；该表记为 `timeout`。`manifest.yaml` 和 `construction.yaml` 的 `iteration_policy`、`steps[*].attempts` 记录配置与实际尝试。
...
- 本地 `viewer.html`：实例关联、本体类型、元数据、构建增量、补充知识和未决项。详情可查看类型 ID、定义和证据；记录节点还显示编号、时间、名称、描述等有限属性预览，每条记录最多 16 项、每项最多 320 字符。默认最多预览 200 个节点。
...
- 字段候选与单列精确核验只覆盖预算内范围。联合行例只来自已核验的原值相等候选；别名模块虽能提出“标准名→别名”线索，尚不能全输入核验、定向或转成可执行关系。定义记录候选也尚未完成概念聚类、同表多类型分派和程序复验。精确频次、稀有样本配额、结构化引用、文本候选、自动条件/复合键提案、转换规则验证和安全计划编译仍待实现；见[状态表](STATUS.md)及[详细设计](FIELD_PROFILING_DESIGN.md)。
